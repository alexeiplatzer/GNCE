...
def check_network_estimates(
    ...
    with open("statistics/" + store_file_name + ".txt", "w") as store_file:
        store_file.write(
            "id,est_card,true_card,enc_avg_time,pred_avg_time,total_avg_time\n"
        )
        for i in range(len(predicted_cardinality)):
            ...
            store_file.write(
                f"{i},{pred_card},{true_card},{encoding_avg_time},{avg_time_prediction_ms},"
    ...
    for i in range(len(predicted_cardinality)):
        ...
        q_err_tmp = max(tmp_card_q_error_changed, q_er_true_card) / min(
            tmp_card_q_error_changed, q_er_true_card
        )
    ...
    final_print_statistics += "maximal q error: prediction %d true %d q-err %.3f\n" % (
        max_error,
        max_nb,
        max(max_error, max_nb) / min(max_error, max_nb),
    )
    final_print_statistics += (
    ...
    for key in errors_per_bucket:
        ...
        if key <= 7:
            ...
            final_print_statistics += (
                ...
                + range_key
                + ": "
                + str(sum_all_vals / len(q_error_per_range[key]))
                + "\n"
            )
            final_print_statistics += (
                "median-q-err-range:"
                + range_key
                + ": "
                + str(np.median(q_error_per_range[key]))
                + "\n"
            )
        final_print_statistics += (
            "average error for bucket %d is %.3f [%d, %d) - number of points in bucket %d\n"
            % (
                key,
                (sum_errors / len(values_per_key)),
                start_bucket,
                end_bucket,
                len(values_per_key),
            )
