...
loading_path = PROJECT_ROOT_PATH
embedding_path = PROJECT_ROOT_PATH
embedding_save_path = PROJECT_ROOT_PATH / "Embeddings"
mapping_save_path = PROJECT_ROOT_PATH / "Embeddings"

file = open(loading_path / "Datasets" / dataset / f"{dataset}.txt", "r")
...
np.save(embedding_save_path / f"{dataset}.emb.npy", lines[:, 1:])

# Save the mapping of the labels to the ids:
with open(mapping_save_path / f"{dataset}_mapping.txt", "w") as f:
    for idd in lines[:, 0]:
        f.write(str(int(idd)))
        f.write("\n")
