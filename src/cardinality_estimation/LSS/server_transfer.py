...
def transfer_queries(dataset):
    ...
    # command = (f"rsync -avz {PROJECT_ROOT_PATH}/data/queryset_homo/{dataset}/paths_1 "
    #            f"schwatkm@{REMOTE_ADDRESS}:/work/schwatkm/lss/data/queryset_homo/{dataset}")
    # # Execute the command
    # subprocess.run(command, shell=True)
    #
    # command = (f"rsync -avz {PROJECT_ROOT_PATH}/data/true_homo/{dataset}/paths_1 "
    #            f"schwatkm@{REMOTE_ADDRESS}:/work/schwatkm/lss/data/true_homo/{dataset}")
    #
    # subprocess.run(command, shell=True)
...
def run_training(dataset_name):
    ...
    #            f' gpcard && cd /work/schwatkm/lss && tmux new-session -d -s mysession && tmux send-keys -t mysession '
    #            f'\\"python active_train.py --dataset {dataset_name} --embed_type prone --mode train --no-cuda'
    #            f'\\" C-m"')
